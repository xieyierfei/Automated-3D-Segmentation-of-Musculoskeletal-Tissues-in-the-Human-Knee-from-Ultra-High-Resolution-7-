...
# Specify directories
test_result_path = 'D:/OAI/results/'
test_path = 'D:/OAI/test_imgs_only/test_slices_im/'
seg_path = 'D:/OAI/test-ground-truth/test_slices_seg/'
...
def test_seg(test_result_path, test_path, seg_path, img_size,
             test_batch_size, save_file, model_weights, tag):
    ...
    for x_test, y_test, fname in img_generator_oai(test_path, seg_path, test_batch_size,
        ...
        dl = np.mean(segutils.calc_dice(y_test, recon))
        dice_losses = np.append(dice_losses, dl)

        cv = np.mean(segutils.calc_cv(y_test, recon))
        ...
        print(' Image #%0.2d (%s). Dice = %0.3f CV = %2.1f VOE = %2.1f VD = %2.1f' % (img_cnt, fname[0:17], dl, cv, voe, vd))
...
if __name__ == '__main__':
    ...
    print('This is the test results in original')
